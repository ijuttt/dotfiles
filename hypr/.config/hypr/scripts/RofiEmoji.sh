...
if [ -n "$selected" ]; then
  # Kasih jeda dikit biar window focus balik if needed
  # sleep 0.1
  wl-copy "$selected"
  wtype "$selected"
fi
...
😋 tounge face savoring food happy joy tongue smile face silly yummy nom delicious savouring
...
👈 kiri backhand index pointing left direction fingers hand left
👉 kanan backhand index pointing right fingers hand direction right
...
🤟 you gesture hand fingers gesture
